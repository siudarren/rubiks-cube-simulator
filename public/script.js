...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getCurrentState(){
        ...
        function findSide(element, side) {
            ...
            for (const [face, normal] of Object.entries(normals)) {
                ...
                const dotProduct = transformedNormal[0] * vector[0] + transformedNormal[1] * vector[1] + transformedNormal[2] * vector[2];
            ...
            function applyMatrix(vector, rotationMatrix) {
                return [
                    vector[0] * rotationMatrix[0][0] + vector[1] * rotationMatrix[1][0] + vector[2] * rotationMatrix[2][0],
                    vector[0] * rotationMatrix[0][1] + vector[1] * rotationMatrix[1][1] + vector[2] * rotationMatrix[2][1],
                    vector[0] * rotationMatrix[0][2] + vector[1] * rotationMatrix[1][2] + vector[2] * rotationMatrix[2][2]
                ];
            }
            ...
            function getRotationMatrix(transform){
                ...
                if (transform.startsWith('matrix3d(')) {
                    ...
                    matrixValues = [
                        temp[0], temp[1], 0, 0,  // m11, m21, m13, m14
                        temp[2], temp[3], 0, 0,  // m12, m22, m23, m24
                        0,       0,       1, 0,  // m31, m32, m33, m34
                        0,       0,       0, 1   // m41, m42, m43, m44
                    ];
                } else if (transform === 'none') {
                    return [[1, 0, 0], [0, 1, 0], [0, 0, 1]];  // No transformation implies 'left' is still 'left'
                }
    ...
    function setUpInitialCubeTilt(xTilt, yTilt){
        ...
        cubeQuaternion = cubeQuaternion.mul(Quaternion.fromAxisAngle([0, -1, 0], xTilt)).mul(Quaternion.fromAxisAngle([1, 0, 0], yTilt));
